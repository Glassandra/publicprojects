...
while(lowestSeed == -1) {
    Int64 iterSeed = iter;
    for(Int64 i = mapsSeed.Length-1; i >= 0; i--) {
        foreach(numberSpan num in mapsSeed[i]) {
            if (iterSeed >= num.span[0] && iterSeed < num.span[0] + num.span[2]) {
                iterSeed = Math.Abs(iterSeed - num.span[0] + num.span[1]);
                break;
            }
        }
    }
    ...
    for(int i = 0; i<seeds.Length; i += 2) {
        if (iterSeed >= seeds[i] && iterSeed < seeds[i] + seeds[i+1]) {
            lowestSeed = iter;
            break;
        }
    }
...
Console.WriteLine("Final resultat is: " + lowestSeed);
